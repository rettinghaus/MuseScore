...
{
    MEIPosition pos = this->findStart(meiElement, measure);
    ...
    if (meiElement.m_name == "breath" || meiElement.m_name == "caesura") {
        ...
        switch (elementType) {
        case (ElementType::PLAYTECH_ANNOTATION): segment = pos.measure->getSegment(SegmentType::ChordRest, pos.tick);
            item = Factory::createPlayTechAnnotation(segment, PlayingTechniqueType::Natural, TextStyleType::STAFF);
            break;
        case (ElementType::STAFF_TEXT): segment = pos.measure->getSegment(SegmentType::ChordRest, pos.tick);
            item = Factory::createStaffText(segment);
            break;
        case (ElementType::SYSTEM_TEXT): segment = pos.measure->getSegment(SegmentType::ChordRest, pos.tick);
            item = Factory::createSystemText(segment);
            break;
        default:
            segment = pos.measure->getSegment(SegmentType::ChordRest, pos.tick);
...
MEIPosition MeiImporter::findStart(const libmei::Element& meiElement, Measure* measure)
{
    MEIPosition pos;
...
MEIPosition MeiImporter::findEnd(pugi::xml_node controlNode, Spanner* spanner)
{
    MEIPosition pos;
...
void MeiImporter::addSpannerEnds()
{
    for (auto spannerMapEntry : m_openSpannerMap) {
        ...
        if (spannerMapEntry.first->isTie()) {
            ...
            MEIPosition pos = this->findEnd(spannerMapEntry.second, spannerMapEntry.first);
            if (!pos.measure) {
                continue;
            }
