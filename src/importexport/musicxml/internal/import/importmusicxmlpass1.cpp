...
static void partGroupStart(MusicXmlPartGroupMap& pgs, int n, int p, const String& s, bool barlineSpan, Color color,
                           const String& name, const String& abbr)
{
                                    ...
                                    MusicXmlPartGroupMap& partGroups)
{
    ...
    constString type = m_e.attribute("type");
    String groupName;
    String groupAbbr;
    String groupSymbol;
    Color symbolColor;
    ...
    while (m_e.readNextStartElement()) {
        if (m_e.name() == "group-name") {
            groupName = m_e.readText();
        } else if (m_e.name() == "group-name-display") {
            groupName = String();
            while (m_e.readNextStartElement()) {
                if (m_e.name() == "display-text") {
                    groupName += m_e.readText();
                } else if (m_e.name() == "accidental-text") {
                    groupName += musicXmlAccidentalTextToChar(m_e.readText());
                } else {
                    skipLogCurrElem();
                }
            }
        } else if (m_e.name() == "group-abbreviation") {
            groupAbbr = m_e.readText();
        } else if (m_e.name() == "group-abbreviation-display") {
            groupAbbr = String();
            while (m_e.readNextStartElement()) {
                if (m_e.name() == "display-text") {
                    groupAbbr += m_e.readText();
                } else if (m_e.name() == "accidental-text") {
                    groupAbbr += musicXmlAccidentalTextToChar(m_e.readText());
                } else {
                    skipLogCurrElem();
                }
            }
    ...
    if (type == "start") {
        partGroupStart(partGroups, number, scoreParts, groupSymbol, barlineSpan, symbolColor, groupName, groupAbbr);
    } else if (type == "stop") {
...
void MusicXmlParserPass1::scorePart(const MusicXmlPartGroupMap& partGroups)
{
    ...
    for (auto const& [number, pg] : partGroups) {
        if (!pg->name.isEmpty() || !pg->abbr.isEmpty()) {
            if (!groupFound || pg->column > maxColumn) {
                maxColumn = pg->column;
                groupName = pg->name;
                groupAbbr = pg->abbr;
                groupFound = true;
    ...
    if (groupFound) {
        if (!groupName.isEmpty()) {
            m_parts[id].setGroupName(groupName);
        }
        if (!groupAbbr.isEmpty()) {
            m_parts[id].setGroupAbbr(groupAbbr);
        }
    }
...
void MusicXmlParserPass1::scoreInstrument(const String& partId, const String& groupName)
{
